...
class Param():
    ...
    def _param_updated(self, pk):
        # print "param_updated h ", pk.get_header(), "p ", pk._get_port(), "d ", pk._get_data(), "d_l ", pk._get_data_l(), "d_t ", pk._get_data_t(), "c ", pk._get_channel()
        varId = pk.datal[0]
        ...
        # print "Calling updateCallBacks cname ", completeName, "s ", s, " element ", element, "varId ", varId
...
class _ParamUpdater(Thread):
    ...
    # this is the helper to call callback when the new pk arrives
    ...
    def request_param_update(self, varid):
        ...
        # print "Requesting update for varid ", varid
        pk = CRTPPacket()
