...
class AiController():
	...
	osc = self.check_oscillation( self.roll_error_array )
	if (osc) :
		amp = self.check_cons_amplitude( self.roll_error_array )
		if ( amp ) :
			period = self.get_period( self.roll_error_array )
			print "Got the period: "
			print period
    ...
    def check_oscillation(self, error_array):
	pos = False
	neg = False
    ...
    def check_cons_amplitude(self, error_array) :
	pos_max = 0
	neg_min = 0
	threshold = 0.5
	
	for error in error_array :
	  if (error > 0 and error > pos_max) :
	    pos_max = error
	  elif (error < 0 and error < neg_min) :
	    neg_min = error
	    
	return threshold > pos_max - (neg_min * -1)

	
    def get_period(self, error_array) :
	pos_max = 0
	x = 0
	y = 0
	...
	for error in error_array :
	  if (x == 0 and error == pos_max) :
	    x = error
	  elif (x != 0 and error == pos_max) :
	    y = error
	    
	return self.time_array[y] - self.time_array[x]
